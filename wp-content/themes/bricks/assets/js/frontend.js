	...
	eachElement: (form) => {
		...
		form.addEventListener('submit', (event) => {
			...
			if (turnstileElement && typeof window.turnstile !== 'undefined') {
				...
				// Queue this submit so Turnstile callback can resume it after a fresh token is generated  (@since 2.x)
				...
				// If previous submit failed, force a fresh token to avoid timeout-or-duplicate errors (@since 2.x)
...
function bricksSubmitForm(elementId, form, files, recaptchaToken, nonceRefreshed) {
	...
	// Current language code (Polylang, WPML) (#86c94gr3q; @since 2.x)
	...
	xhr.onreadystatechange = function () {
		...
		if (res.success) {
			...
			let turnstileElement = form.querySelector('.cf-turnstile')
			// Get current Turnstile widget reference (@since 2.x)
			...
			if (turnstileElement && typeof window.turnstile !== 'undefined' && turnstileWidgetRef) {
				...
				// Flag next submit to force token regeneration instead of reusing the failed token (@since 2.x)
				form.dataset.brxTurnstileRefresh = 'true'

				if (turnstileHiddenInput) {
					// Keep hidden response in sync with reset state (@since 2.x)
...
const bricksAnchorLinksFn = new BricksFunction({
	...
	frontEndOnly: true,
	eachElement: (anchor) => {
		// Normalize pathname by removing trailing slashes for accurate comparison (@since 2.x)
		...
		// Determine if the anchor link is for the current page by comparing the origin and normalized pathname (ignoring trailing slashes) (@since 2.x)
